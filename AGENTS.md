...
1. **MCP Server** (`@regground/mcp-server`) — The primary distribution mechanism.
   Any MCP-compatible tool (Codex, Cursor, Windsurf, custom agents) can call
...
The platform targets frontier models (Codex, GPT, Gemini, future AGI) via a
...
**Transports:** stdio (for Codex / Cursor / IDE integration) and HTTP (for
